...
const CustomerDetails = () => {
  ...
  dispatch(setPaymentDetails(paymentDetailsResponse.data))
        ...
        dispatch({ type: 'SET_ERROR', payload: 'Error fetching customer details. Please try again later.' });
      } finally {
  ...
  const fetchName = async (endpoint, ...ids) => {
    try {
      ...
      console.log('Project api response is ' , response)
  ...
  const fetchUnitDetails = async (projectId, blockId, unitId) => {
    try {
      ...
      console.log('unit details response are ' , response)
      ...
      <table id='firsttable'>
        ...
        <tbody>
          ...
          <td><strong>Total Received Payment</strong></td>
            <td>{calculateTotalAmounts(data)}</td>
            <td><strong>Balance</strong></td>
            <td>{total - calculateTotalAmounts(data)}</td>
          </tr>
        </tbody>
      </table>
    )}
