...
import { fetchProjects } from "../services/customerService";
import useFetchUser from "../hooks/useFetchUser";
const SubadminDashboardproject = () => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        if (response.status === 200 && data.status === "ok") {
          ...
          const filteredProjects = projectsWithUnitCount.filter(
            (project) => project.name === subAdmin.AssgProject
          );
...
export default SubadminDashboardproject;